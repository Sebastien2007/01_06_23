#######################LICENCED BY MIT################################
###......................J.K.IT'S..LICENCED..BY..MY..DICK..HA-HA-HA###
import socket
import pickle
import struct
import cv2
...
import numpy as np
#import requests
dick = cv2.aruco.getPredefinedDictionary(cv2.aruco.DICT_4X4_250)
...
while True:
    ...
    try:
        Index=np.where(res[1]==143)[0][0]
        cntr=res[0][Index][0][143].astype(np.int16)
        cv2.circle(frame, cntr, 10, (0,0,255), thickness=-1)
    except:
        print('no cube suckaaAaaA')
